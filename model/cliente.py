...
def cadastrar_cliente(nome, telefone, sexo, data_nascimento, email, senha):
    ...
    if not telefone_str.isdigit() or len(telefone_str) != 11:
        print(f"Erro: telefone '{telefone}' inválido (deve conter 11 dígitos numéricos)")
        return {"erro": "O telefone deve conter exatamente 11 dígitos numéricos."}


    if not data_nascimento or not sexo or str(sexo).strip() == "":
        print(f"Erro: sexo ou data_nascimento inválido")
        return {"erro": "Sexo ou data de nascimento inválido. Recebido: sexo='{}', data_nascimento='{}'".format(sexo, data_nascimento)}
    ...
    try:
        ...
        return {"erro": f"Data de nascimento '{data_nascimento}' inválida. Esperado formato YYYY-MM-DD."}


    hoje = datetime.today().date()
    idade = hoje.year - data_nascimento_date.year - ((hoje.month, hoje.day) < (data_nascimento_date.month, data_nascimento_date.day))
    if idade < 10 or idade > 99:
        print(f"Erro: idade {idade} fora do permitido (10-99 anos)")
        return {"erro": "A idade deve ser entre 10 e 99 anos."}
    ...
    if conn:
        ...
        try:
            ...
            if cursor.fetchone():
                ...
                return {"erro": f"Já existe uma conta com este e-mail: {email}"}
            ...
            if cursor.fetchone():
                ...
                return {"erro": f"Já existe uma conta com este número de telefone: {telefone}"}
            ...
            if not result:
                print("Erro: Falha ao inserir cliente no banco de dados.")
                return {"erro": "Falha ao inserir cliente no banco de dados."}
            cliente_id = result[0]
            ...
            try:
                from controller.email_api import send_confirmation_email
            ...
            return {"erro": f"Falha ao cadastrar cliente: {str(e)}"}
        finally:
            if cursor:
                cursor.close()
            conn.close()
        ...
        try:
            ...
            try:
                ...
                if cliente:
                    ...
                    try:
                        from controller.email_api import sendgrid_email_api_massoterapia
